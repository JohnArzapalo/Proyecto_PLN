...
class RAGComponent:
    ...
    def __init__(self, db_path: str = "./hannah_vectordb",
        ...
        # ─── Embedder compartido ─────────────────────────────────────
        # Un solo modelo de embeddings que comparten VectorStore,
        # SemanticCache y ContextHandler → solo se carga una vez.
        shared_embedder = EmbeddingService()

        # ─── Componente 1: Base de datos vectorial ───
        # Almacena los documentos como vectores en ChromaDB
        self.vector_store = VectorStore(db_path=db_path, embedder=shared_embedder)
        ...
        self.context_handler = ContextHandler(embedder=shared_embedder)
    ...
    def retrieve(self, query: str, mode: str = "simplified") -> dict:
        ...
        # ═══════════════════════════════════════════════════
        # GUARD: Empty knowledge base → skip retrieval
        # ═══════════════════════════════════════════════════
        if self.vector_store.collection.count() == 0:
            print("[RAG] Knowledge base is empty — skipping retrieval.")
            return {
                "formatted_context": "[MEMORY][/MEMORY]",
                "raw_chunks": [],
                "scores": [],
                ...
                "approx_tokens": 0,
                "cache_hit": False,
                "enhanced_query": {"original": query, "mode": mode},
            }
    ...
    def _multi_query_search(self, queries: list[str], n_results: int) -> dict:
        ...
        available = self.vector_store.collection.count()
